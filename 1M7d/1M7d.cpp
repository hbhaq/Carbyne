...
int rbf_check, cbf_mempool_lookup, expiry_hours=24*7, non_block_not_removed=0;
...
bf::counting_bloom_filter cbf_mempool_one(make_hasher(14,time(0), true), 4000000, width);
   ...
   // debug_rbf << "total_rbf"<<","<< "rbf_missed" << "," << "rbf_replaced" << "," <<"% replaced total"<<","<< "h_rbf" <<","<<"h_rbf_missed"<<","<<"h_rbf_replaced"<<","<<"% replaced hour"<<endl;
    ...
    for (int j = 1; j <= 183; j++) {
        ...
        /*    if (mempool_time - 3600 > last_clear_time) { 
           ...
           /*     auto gd = get_data.begin();
                while (gd != get_data.end()) {
                    if (gd->second < (mempool_time - (3600 )))   // Check if value of this entry matches with given value
                        get_data.erase(gd->first); // erase
		...
		if (current_mempool_one>= 200000 || current_mempool_two>=200000) {  // rotate
            ...
            if (type == "ENTRY") {
                ...
                if (mempool_time > start_timestamp) {
                    ...
                    if (shadow_inventory.count(tx->getHash().ToString()) == 1 &&
                        inventory_positives.count(tx->getHash().ToString()) == 0 /*&& shadow_mempool.count(tx->getHash().ToString())==0*/) {
			...
			if (hour!=1468){
                        ...
                        confusion_matrix_cbf_mempool_entry(tx->getType(), tx->getHash().ToString(),
                                                           shadow_mempool_lookup, cbf_mempool_lookup, tx_indices);
			}
				...
				shadow_cbf.insert(make_pair(tx->getHash().ToString(), 1));
				    ...
				    int cbf_mempool_lookup_one = cbf_mempool_one.lookup(tx->getHash().ToString());
                           ...
                           // status = confusion_matrix_cbf_mempool_exit(tx->getType(), tx->getReason(),
                             //                                          tx->getHash().ToString(),
            ...
            if (mempool_time > start_timestamp) {
                ...
                if (mempool_time - last_debug_time >= 3600) {
                    ...
                    h_cm_entry = h_tp_entry + h_tn_entry + h_fp_entry + h_fn_entry + h_tp_exit + h_fp_exit + h_fn_exit;
                  ...
                  debug_cumulative << hour << "," << (start_timestamp + 3600 * hour)
                                     ...
                                     << "," << current_mempool_tx-shadow_mempool.size()+(fp_entry-fn_entry+fp_exit-fn_exit)
